...
from astrowidgets.cursor_info import READOUT_TEMPLATE, format_cursor_text
...
def test_format_decimal_readout_is_fixed_width(wcs):
    ...
    lengths = {
        len(format_cursor_text(x, 2.0, data=big_data, wcs=wcs))
...
def test_sky_format_change_updates_readout_immediately(backend, wcs):
    ...
    expected = READOUT_TEMPLATE.format(
        format_cursor_text(
            3.0, 2.0, data=data, wcs=wcs, sky_format="sexagesimal"
        )
    )
    assert widget._cursor_readout.value == expected
